...
> **수정 상수**: `MQ135_R0_KOHM` (현재 추정값: 44.7 kΩ)
> **현재 상태**: R0는 raw ≈ 240 기준으로 추정된 값. 실환경 측정으로 정확도 개선 필요.
...
```
Rs_clean_air = RL × (VCC − V) / V
R0 = Rs_clean_air / 3.6        ← 3.6: MQ-135 데이터시트의 clean-air Rs/R0 비율
```
...
| 항목 | 사양 | 비고 |
|------|------|------|
| 디바이스 | XIAO ESP32-C6 + MQ-135 배선 완료 | GPIO0(A0) ← AOUT |
